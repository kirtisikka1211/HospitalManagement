...
final case class Pharmacy(id: String, patientId: String, doctorId: String, date: String, time: String, medication: String)
...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    def docFromPharmacy(p: Pharmacy): Document = 
      Document(  "id" -> p.id, "patientId" -> p.patientId, "doctorId" -> p.doctorId, "date" -> p.date, "time" -> p.time, "medication" -> p.medication)
